# NewBeePlus 能效标签与缺陷检测系统 — 数据库设计文档
...
```
┌──────────────────┐        ┌──────────────────────┐
│    products      │        │   detection_records   │
│    (产品表)       │        │    (检测记录表)        │
├──────────────────┤        ├──────────────────────┤
│ PK id (AUTO)     │        │ PK id (AUTO)         │
│    model         │        │    timestamp          │
│    energy_grade  │        │    product_model      │
│    power_consump │        │    grade              │
└──────────────────┘        │    energy_param       │
                            │    standby_power      │
                            │    is_data_match      │
...
**默认数据**：
```sql
INSERT INTO products (model, energy_grade, power_consumption) VALUES ('HB-2024-001', 1, 120);
INSERT INTO products (model, energy_grade, power_consumption) VALUES ('HB-2024-002', 2, 150);
INSERT INTO products (model, energy_grade, power_consumption) VALUES ('HB-2024-003', 3, 180);
```

### 4.2 检测记录表（detection_records）
...
| energy_param | TEXT | DEFAULT '未识别' | 能效参数 |
| standby_power | TEXT | DEFAULT '未识别' | 待机功率 |
| is_data_match | INTEGER | DEFAULT 0 | 数据是否匹配（0/1） |
| defects | TEXT | DEFAULT '{}' | 缺陷信息（JSON 字符串） |
| position | TEXT | DEFAULT '{}' | 位置信息（JSON 字符串） |
| is_pass | INTEGER | DEFAULT 0 | 是否合格（0/1） |
| has_defect | INTEGER | DEFAULT 0 | 是否有缺陷（0/1） |
